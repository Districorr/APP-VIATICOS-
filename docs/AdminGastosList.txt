...
// --- CAMBIO 1: Importamos la nueva función genérica ---
...
async function fetchData() {
  ...
  errorMessage.value = '';
  try {
    const { data, error } = await supabase.rpc('get_detalle_rendicion_con_gastos', { p_viaje_id: viajeId });
...
const filteredAndSortedGastos = computed(() => {
  ...
  if (filters.value.tipoGastoId) {
    items = items.filter(g => String(g.tipo_gasto_id) === String(filters.value.tipoGastoId));
  }
...
const kpis = computed(() => {
    const gastosVisibles = filteredAndSortedGastos.value;
    const totalGastado = gastosVisibles.reduce((sum, g) => sum + Number(g.gasto_monto_total || 0), 0);
    const adelanto = Number(rendicion.value?.monto_adelanto || 0);
    const saldo = adelanto - totalGastado;
    return { adelanto, gastado: totalGastado, saldo, numGastos: gastosVisibles.length };
});

const totalesPorCategoria = computed(() => {
    const result = {};
    filteredAndSortedGastos.value.forEach(gasto => {
        const categoria = gasto.nombre_tipo_gasto || 'Sin Categoría';
        if (!result[categoria]) {
            result[categoria] = { total: 0, count: 0 };
        }
        result[categoria].total += Number(gasto.gasto_monto_total || 0);
...
// --- LÓGICA DE ACCIONES ---
// (La mayoría de las funciones aquí no cambian)
const setSortBy = (key) => {
...
const handleEstadoRevisionChange = async (gastoId, nuevoEstado) => {
    ...
    try {
        ...
        if (error) throw error;
        const gastoIndex = gastos.value.findIndex(g => g.gasto_id === gastoId);
...
const saveComment = async () => {
    ...
    if (!gasto) return;
    actionLoading.value = `comment_${gasto.gasto_id}`;
    try {
        const { error } = await supabase.rpc('actualizar_estado_gasto', {
        ...
        if (error) throw error;
        const gastoIndex = gastos.value.findIndex(g => g.gasto_id === gasto.gasto_id);
        if (gastoIndex !== -1) {
...
const editarGasto = (gastoId) => {
    router.push({ name: 'GastoFormEdit', params: { id: gastoId } });
};

const duplicarGasto = (gasto) => {
    const gastoData = { ...gasto };
    delete gastoData.gasto_id;
    delete gastoData.gasto_created_at;
...
const handleExport = () => {
  if (!filteredAndSortedGastos.value || filteredAndSortedGastos.value.length === 0) {
    alert('No hay gastos para exportar con los filtros actuales.');
    return;
  }
  const dataToExport = filteredAndSortedGastos.value.map(g => ({
    'ID Gasto': g.gasto_id,
    'Fecha': formatDate(g.fecha_gasto),
    'Responsable': g.responsable_gasto_nombre,
    'Tipo Gasto': g.nombre_tipo_gasto,
    'Descripción': g.gasto_descripcion,
    'Monto': g.gasto_monto_total,
    'Moneda': g.gasto_moneda,
    'Factura N°': g.gasto_n_factura,
    'Estado Revisión': g.estado_revision,
    'Comentario Admin': g.comentario_revision,
    'Rendición (#)': `${g.nombre_viaje} (#${g.viaje_codigo_rendicion})`,
  }));

  // --- CAMBIO 2: Llamamos a la nueva función correcta ---
...
<template>
  <div class="container mx-auto px-2 sm:px-4 lg:px-6 py-8">
    ...
    <div v-else-if="rendicion">
          ...
          <table class="min-w-full divide-y divide-gray-200">
            <thead class="bg-gray-100">
              <tr>
                ...
                <th class="table-th"><button @click="setSortBy('fecha_gasto')" class="sort-button">Fecha <ArrowDownIcon v-if="sortBy.key === 'fecha_gasto' && sortBy.order === 'desc'" class="h-4 w-4" /><ArrowUpIcon v-if="sortBy.key === 'fecha_gasto' && sortBy.order === 'asc'" class="h-4 w-4" /></button></th>
                <th class="table-th">Tipo Gasto</th>
                <th class="table-th">Descripción</th>
                <th class="table-th w-56">Detalles</th>
                <th class="table-th text-right"><button @click="setSortBy('gasto_monto_total')" class="sort-button justify-end w-full">Monto <ArrowDownIcon v-if="sortBy.key === 'gasto_monto_total' && sortBy.order === 'desc'" class="h-4 w-4" /><ArrowUpIcon v-if="sortBy.key === 'gasto_monto_total' && sortBy.order === 'asc'" class="h-4 w-4" /></button></th>
            ...
            <tbody class="bg-white divide-y divide-gray-200">
              ...
              <tr v-for="gasto in filteredAndSortedGastos" :key="gasto.gasto_id" class="hover:bg-gray-50/75 transition-colors">
                <td class="table-cell px-4 py-3"><Menu as="div" class="relative inline-block text-left"><MenuButton class="w-full text-xs font-semibold rounded-md px-2 py-1.5 inline-flex items-center justify-center gap-1.5 transition-colors" :class="{'bg-green-100 text-green-800 hover:bg-green-200': gasto.estado_revision === 'aprobado', 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200': gasto.estado_revision === 'observado', 'bg-gray-100 text-gray-700 hover:bg-gray-200': gasto.estado_revision === 'en_revision'}"><span v-if="actionLoading === `revision_${gasto.gasto_id}`" class="h-4 w-4 border-2 border-gray-400 border-t-transparent rounded-full animate-spin"></span><template v-else>{{ gasto.estado_revision.replace('_',' ') }}</template><ChevronDownIcon class="h-4 w-4"/></MenuButton><MenuItems class="absolute left-0 mt-2 w-40 origin-top-left rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none z-10"><div class="px-1 py-1"><MenuItem><button @click="() => handleEstadoRevisionChange(gasto.gasto_id, 'aprobado')" class="menu-item text-green-600">Aprobado</button></MenuItem><MenuItem><button @click="() => handleEstadoRevisionChange(gasto.gasto_id, 'en_revision')" class="menu-item text-gray-700">En Revisión</button></MenuItem><MenuItem><button @click="() => handleEstadoRevisionChange(gasto.gasto_id, 'observado')" class="menu-item text-yellow-600">Observado</button></MenuItem></div></MenuItems></Menu></td>
                <td class="table-cell">{{ formatDate(gasto.fecha_gasto) }}</td>
                <td class="table-cell"><span class="px-2 py-1 text-xs font-semibold rounded-md bg-slate-100 text-slate-700">{{ gasto.nombre_tipo_gasto }}</span></td>
                <td class="table-cell max-w-xs"><div class="truncate" :title="gasto.gasto_descripcion">{{ gasto.gasto_descripcion || '-' }}</div></td>
                <td class="table-cell"><DetallesJson :datos="gasto.gasto_datos_adicionales" /></td>
                <td class="table-cell text-right font-semibold" :class="{ 'text-red-500 font-bold': gasto.gasto_monto_total > 100000 }">{{ formatCurrency(gasto.gasto_monto_total, gasto.gasto_moneda) }}</td>
                <td class="table-cell text-center"><div class="flex justify-center items-center gap-3"><a v-if="gasto.gasto_factura_url" :href="gasto.gasto_factura_url" target="_blank" class="text-blue-600 hover:text-blue-800" title="Ver factura"><PaperClipIcon class="h-5 w-5"/></a><button @click="() => openCommentModal(gasto)" class="text-gray-500 hover:text-gray-800" title="Comentarios"><ChatBubbleLeftEllipsisIcon class="h-5 w-5" :class="{'text-yellow-500': gasto.comentario_revision}"/></button></div></td>
      ...
      <section class="mt-8">
        <h3 class="text-xl font-semibold text-gray-800 mb-4">Totales por Categoría (Filtro Actual)</h3>
    ...
    <transition name="modal-fade">
      <div v-if="commentModal.show" class="modal-backdrop" @click.self="commentModal.show = false">
        <div class="modal-container">
          ...
          <p class="text-sm text-gray-600 mt-1">Para el gasto: <span class="font-semibold">{{ commentModal.gasto?.gasto_descripcion }}</span></p>
          ...
          <div class="modal-actions">
            ...
            <button @click="saveComment" :disabled="actionLoading === `comment_${commentModal.gasto?.gasto_id}`" class="btn-primary flex justify-center w-24">
              <span v-if="actionLoading === `comment_${commentModal.gasto?.gasto_id}`" class="h-5 w-5 border-2 border-white border-t-transparent rounded-full animate-spin"></span>
...
.btn-secondary-outline { @apply bg-white text-gray-700 font-semibold py-2 px-4 rounded-lg border border-gray-300 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors; }
.btn-icon { @apply p-2 rounded-full text-gray-500 hover:bg-gray-100 hover:text-gray-800 transition-colors; }
...
.modal-error { @apply mt-3 text-sm text-red-600 font-medium; }
