...
// Importaciones para XLS, etc. (se mantienen las que ya tenías)
import * as XLSX from 'xlsx';

// --- NUEVA FUNCIÓN REUTILIZABLE PARA OBTENER DATOS DEL REPORTE ---
// Esta función será llamada tanto por la vista previa como por el generador de PDF.
const getReportData = async (viajeId) => {
  if (!viajeId) {
    throw new Error("No se ha proporcionado un ID de rendición para obtener los datos.");
  }
  const { data: reporte, error } = await supabase.rpc('get_reporte_rendicion_completa', { p_viaje_id: viajeId });
  
  if (error) {
    console.error("Error en RPC get_reporte_rendicion_completa:", error);
...
// Reemplaza esta función en src/composables/useReportGenerator.js
// Pega esta función completa dentro de src/composables/useReportGenerator.js

const generateAdminOperativoPDF = async (kpis, tables, filters, outputType = 'save') => {
  ...
  try {
    ...
    // --- Helper para Header y Footer ---
    const addPageHeaderAndFooter = (doc, pageNum, totalPages) => {
      // Footer
      doc.setFontSize(8).setTextColor(150);
      doc.text(`Página ${pageNum} de ${totalPages} | InfoGastos Districorr`, pageWidth / 2, pageHeight - 10, { align: 'center' });
    };

    // ===============================================
    // --- PÁGINA 1: PORTADA ---
    // ===============================================
    doc.setFontSize(24).setFont(undefined, 'bold').setTextColor(40, 56, 104);
    doc.text("Reporte Operativo Administrativo", pageWidth / 2, lastY + 20, { align: 'center' });
    
    lastY += 50;
    
    doc.setFontSize(11).setFont(undefined, 'normal').setTextColor(100);
    doc.text(`Fecha de Generación:`, margin, lastY);
    doc.setFont(undefined, 'bold').setTextColor(40);
    doc.text(formatDate(new Date(), { day: '2-digit', month: 'long', year: 'numeric' }), pageWidth - margin, lastY, { align: 'right' });
    lastY += 10;

    doc.setFontSize(11).setFont(undefined, 'normal').setTextColor(100);
    doc.text(`Período Analizado:`, margin, lastY);
    doc.setFont(undefined, 'bold').setTextColor(40);
    doc.text(`${formatDate(filters.dateRange.from)} al ${formatDate(filters.dateRange.to)}`, pageWidth - margin, lastY, { align: 'right' });
    lastY += 10;

    doc.setFontSize(11).setFont(undefined, 'normal').setTextColor(100);
    doc.text(`Granularidad:`, margin, lastY);
    doc.setFont(undefined, 'bold').setTextColor(40);
    doc.text(filters.granularity.charAt(0).toUpperCase() + filters.granularity.slice(1), pageWidth - margin, lastY, { align: 'right' });
    
    // ===============================================
    // --- PÁGINA 2 (O SIGUIENTE): KPIs Y TABLAS ---
    // ===============================================
    doc.addPage();
    lastY = margin;

    doc.setFontSize(16).setFont(undefined, 'bold').setTextColor(40, 56, 104);
    doc.text("Indicadores Clave (KPIs)", margin, lastY);
    lastY += 6;

    const kpiBody = [
      [{ content: 'Gasto Total del Período', styles: { fontStyle: 'bold' } }, formatCurrency(kpis.gastoTotal)],
      [{ content: 'Gasto Promedio Diario', styles: { fontStyle: 'bold' } }, formatCurrency(kpis.gastoPromedioDiario)],
      [{ content: 'Top Tipo de Gasto', styles: { fontStyle: 'bold' } }, `${kpis.topTipoGasto.nombre} (${formatCurrency(kpis.topTipoGasto.monto)})`],
      [{ content: 'Top Cliente', styles: { fontStyle: 'bold' } }, `${kpis.topCliente.nombre} (${formatCurrency(kpis.topCliente.monto)})`],
    ];
    if (kpis.variacionVsPeriodoAnterior !== null) {
      kpiBody.push([{ content: 'Variación vs. Período Anterior', styles: { fontStyle: 'bold' } }, `${kpis.variacionVsPeriodoAnterior.toFixed(2)}%`]);
    }
    ...
    // --- Helper para renderizar tablas con salto de página automático ---
    const renderTable = (title, head, body) => {
      // Estima la altura de la sección (título + header + N filas)
      const estimatedHeight = 10 + 8 + (body.length * 7) + 5; // Ajuste fino de altura
      if (lastY + estimatedHeight > pageHeight - margin) {
        doc.addPage();
        lastY = margin;
      }
      doc.setFontSize(14).setFont(undefined, 'bold').setTextColor(40, 56, 104);
      ...
      doc.autoTable({
        ...
        body,
        theme: 'striped',
        headStyles: { fillColor: [230, 230, 230], textColor: 40, fontStyle: 'bold' },
        styles: { fontSize: 8, cellPadding: 1.5 },
        columnStyles: {
          // Ajuste para que las columnas de montos se alineen a la derecha
          2: { halign: 'right' },
          3: { halign: 'right' },
        }
      });
    ...
    if (tables.totalesPorPeriodo && tables.totalesPorPeriodo.length > 0) {
      renderTable(
        'Totales por Período',
        ['Período', 'Cant. Gastos', 'Monto Total', 'Ticket Promedio'],
        tables.totalesPorPeriodo.map(r => [r.periodo, r.cantidad_gastos, formatCurrency(r.monto_total), formatCurrency(r.ticket_promedio)])
      );
    }
    if (tables.porTipoGasto && tables.porTipoGasto.length > 0) {
      renderTable(
        'Desglose por Tipo de Gasto',
        ['Tipo de Gasto', 'Cant.', 'Monto', '% del Total'],
        tables.porTipoGasto.map(r => [r.Tipo, r.cantidad, formatCurrency(r.monto), `${r.porcentaje.toFixed(2)}%`])
      );
    }
    if (tables.porResponsable && tables.porResponsable.length > 0) {
      renderTable(
        'Desglose por Responsable',
        ['Responsable', 'Cant.', 'Monto', '% del Total'],
        tables.porResponsable.map(r => [r.Responsable, r.cantidad, formatCurrency(r.monto), `${r.porcentaje.toFixed(2)}%`])
      );
    }
    if (tables.porCliente && tables.porCliente.length > 0) {
      renderTable(
        'Desglose por Cliente',
        ['Cliente', 'Cant.', 'Monto', '% del Total'],
        tables.porCliente.map(r => [r.Cliente, r.cantidad, formatCurrency(r.monto), `${r.porcentaje.toFixed(2)}%`])
      );
    }
    if (tables.porProvincia && tables.porProvincia.length > 0) {
      renderTable(
        'Desglose por Provincia',
        ['Provincia', 'Cant.', 'Monto', '% del Total'],
        tables.porProvincia.map(r => [r.Provincia, r.cantidad, formatCurrency(r.monto), `${r.porcentaje.toFixed(2)}%`])
      );
    }

    // ===============================================
    // --- FINALIZACIÓN: PAGINACIÓN ---
    // ===============================================
    ...
    console.error("Error al generar el PDF operativo:", e);
...
const generateCanvaStylePDF = async (viajeId) => {
  try {
    // 1. Obtener los datos usando la función RPC (esta ya está funcionando bien)
    ...
    // --- ENCABEZADO Y METADATA (Sin cambios) ---
      ...
      for (const grupo of reporte.grupos) {
        ...
        if (grupo.gastos && Array.isArray(grupo.gastos)) {
          grupo.gastos.forEach(gasto => {
              ...
              const detallesAdicionales = gasto.detalles_adicionales || {};
              if (detallesAdicionales.origen && detallesAdicionales.origen !== ' - ') detalles.push(`Origen: ${detallesAdicionales.origen}`);
              if (detallesAdicionales.destino && detallesAdicionales.destino !== ' - ') detalles.push(`Destino: ${detallesAdicionales.destino}`);
              if (detallesAdicionales.lugar_gasto && detallesAdicionales.lugar_gasto !== ' - ') detalles.push(`Lugar del Gasto: ${detallesAdicionales.lugar_gasto}`);
              if (detallesAdicionales.transporte) detalles.push(`Transporte: ${detallesAdicionales.transporte}`);
              if (detallesAdicionales.proveedor) detalles.push(`Proveedor: ${detallesAdicionales.proveedor}`);
              if (detallesAdicionales.numero_factura) detalles.push(`Nº Factura: ${detallesAdicionales.numero_factura}`);
...
// --- FUNCIÓN DE GENERACIÓN DE PDF DEL REPORTE DE MOVIMIENTOS DE CAJA ---
  const generateCajaReportePDF = async (cajaId, fechaDesde, fechaHasta) => {
    try {
      // 1. Llamar a la RPC actualizada que ahora incluye el resumen por provincia
      const { data: reporte, error } = await supabase.rpc('get_reporte_caja_completo', {
        p_caja_id: cajaId,
        p_fecha_desde: fechaDesde,
        p_fecha_hasta: fechaHasta
      });

      if (error) throw new Error(`Error al obtener datos del reporte: ${error.message}`);
      if (!reporte || !reporte.movimientos || reporte.movimientos.length === 0) {
        throw new Error("No se encontraron movimientos en el período seleccionado para generar el reporte.");
      }
      ...
      if (logoImg) doc.addImage(logoImg, 'PNG', margin, margin - 6, 16, 16);
      doc.setFontSize(20).setFont(undefined, 'bold').setTextColor(...primaryColor);
      doc.text("REPORTE DE MOVIMIENTOS DE CAJA", pageWidth / 2, margin + 2, { align: 'center' });
      doc.setFontSize(8).setFont(undefined, 'normal').setTextColor(100, 100, 100);
      doc.text("SISTEMA INFOGASTOS – DISTRICORR", pageWidth / 2, margin + 7, { align: 'center' });
      
      const metadataBody = [
        ['Caja:', reporte.metadata.caja_nombre, 'Fecha Emisión:', formatDate(new Date())],
        ['Responsable:', reporte.metadata.responsable, '', ''],
        ['Período:', `${reporte.metadata.periodo_desde} al ${reporte.metadata.periodo_hasta}`, '', ''],
      ];
      ...
      doc.autoTable({
        ...
        body: metadataBody,
        theme: 'plain',
        styles: { fontSize: 8, cellPadding: {top: 0.2, bottom: 0.2} },
        columnStyles: { 0: { fontStyle: 'bold' }, 2: { fontStyle: 'bold' } }
      });
      lastY = doc.lastAutoTable.finalY + 4;

      // --- ENCABEZADO TIPO "PANEL DE CONTROL" ---
      const summaryData = [
        [
          { content: `Saldo Inicial\n${formatCurrency(reporte.resumen_financiero.saldo_inicial)}`, styles: { halign: 'center', fontStyle: 'bold', fontSize: 9 } },
          { content: `Total Ingresos\n${formatCurrency(reporte.resumen_financiero.total_ingresos)}`, styles: { halign: 'center', fontStyle: 'bold', fontSize: 9, textColor: [0, 100, 0] } },
          { content: `Total Egresos\n${formatCurrency(reporte.resumen_financiero.total_egresos)}`, styles: { halign: 'center', fontStyle: 'bold', fontSize: 9, textColor: [192, 0, 0] } },
          { content: `Saldo Final\n${formatCurrency(reporte.resumen_financiero.saldo_final)}`, styles: { halign: 'center', fontStyle: 'bold', fontSize: 10, fillColor: [224, 224, 224] } },
        ]
      ];
      doc.autoTable({
        startY: lastY,
      ...
      doc.setFontSize(9).setFont(undefined, 'bold').setTextColor(255, 255, 255);
      doc.text("Detalle de Movimientos", margin + 2, lastY + 5);
      lastY += 7;

      const tableBody = reporte.movimientos.map(mov => {
        let descripcion = mov.descripcion || '-';
        if (mov.localidad) {
          descripcion += ` (${mov.localidad})`;
        }
        return [
          ...
          mov.categoria || '-',
          descripcion,
          { content: mov.tipo_movimiento !== 'gasto' ? formatCurrency(mov.monto) : '', styles: { textColor: [0, 100, 0] } },
          { content: mov.tipo_movimiento === 'gasto' ? formatCurrency(mov.monto) : '', styles: { textColor: [192, 0, 0] } },
          { content: formatCurrency(mov.saldo_posterior), styles: { fontStyle: 'bold' } }
        ];
      });

      doc.autoTable({
        startY: lastY,
        head: [['Fecha', 'Tipo', 'Categoría', 'Descripción', 'Ingreso', 'Egreso', 'Saldo']],
        body: tableBody,
        theme: 'grid',
        headStyles: { fillColor: [224, 224, 224], textColor: [40, 40, 40], fontStyle: 'bold', fontSize: 8, cellPadding: 1.5 },
        bodyStyles: { textColor: [0, 0, 0], fontSize: 8, cellPadding: 1.5, valign: 'middle' },
        columnStyles: {
          3: { cellWidth: 'auto' },
      ...
      const summaryTableConfig = { startY: lastY + 8, theme: 'grid', headStyles: { fillColor: primaryColor, fontSize: 8 }, styles: { fontSize: 7, cellPadding: 1.2 } };
      const columnWidth = (pageWidth - margin * 2) / 2 - 2;
      let finalY = lastY;

      if (reporte.resumen_por_categoria?.length > 0) {
        const resumenCategoriaBody = reporte.resumen_por_categoria.map(item => [item.categoria, formatCurrency(item.monto), `${item.porcentaje || 0}%`]);
        doc.autoTable({ ...summaryTableConfig, tableWidth: columnWidth, head: [['Gastos por Categoría', 'Monto', '%']], body: resumenCategoriaBody, margin: { left: margin }, columnStyles: { 1: { halign: 'right' }, 2: { halign: 'right' } } });
        finalY = Math.max(finalY, doc.lastAutoTable.finalY);
      }

      // --- INICIO DE LA CORRECCIÓN ---
      if (reporte.resumen_por_provincia?.length > 0) {
        const resumenProvinciaBody = reporte.resumen_por_provincia.map(item => [item.provincia, formatCurrency(item.monto), `${item.porcentaje || 0}%`]);
        // Usamos la misma posición vertical (startY) pero un margen izquierdo diferente para colocarla al lado
        doc.autoTable({ ...summaryTableConfig, startY: lastY + 8, tableWidth: columnWidth, head: [['Gastos por Provincia', 'Monto', '%']], body: resumenProvinciaBody, margin: { left: margin + columnWidth + 4 }, columnStyles: { 1: { halign: 'right' }, 2: { halign: 'right' } } });
        finalY = Math.max(finalY, doc.lastAutoTable.finalY);
      }
      // --- FIN DE LA CORRECCIÓN ---
      lastY = finalY;
      ...
      // --- FIRMAS ---
      lastY += 15;
      if (lastY > pageHeight - 25) { doc.addPage(); lastY = margin + 10; }
      doc.setFontSize(8).setTextColor(0,0,0);
      doc.text("Firma Responsable", margin + 35, lastY, {align: 'center'});
      doc.line(margin, lastY - 2, margin + 70, lastY - 2);
      doc.text("Firma Gerencia", pageWidth - margin - 35, lastY, {align: 'center'});
      doc.line(pageWidth - margin - 70, lastY - 2, pageWidth - margin, lastY - 2);
      ...
      doc.save(`Reporte_Caja_${reporte.metadata.caja_nombre.replace(/\s/g, '_')}_${fechaDesde}_${fechaHasta}.pdf`);
    ...
    // --- NUEVA FUNCIÓN GENÉRICA PARA EXPORTAR JSON A EXCEL ---
  /**
   * Exporta un array de objetos JSON a un archivo .xlsx.
   * @param {Array<Object>} jsonData - El array de objetos a exportar. Las keys de los objetos serán los encabezados.
   * @param {string} fileNamePrefix - El prefijo del nombre del archivo a generar.
   */
  const exportJsonToExcel = (jsonData, fileNamePrefix = 'reporte') => {
    ...
    try {
      ...
      // Adjunta la hoja de cálculo al libro con el nombre 'Datos'.
      ...
      alert("Ocurrió un error al intentar generar el archivo Excel. Revisa la consola para más detalles.");
    }
  };

    // --- NUEVA FUNCIÓN PARA EXPORTAR LA LISTA DE RENDICIONES ---
  const generateRendicionesListPDF = (rendiciones, filtros) => {
    if (!rendiciones || rendiciones.length === 0) {
      alert("No hay rendiciones en la lista para generar un reporte.");
      return;
    }
    ...
    doc.setFontSize(16).setFont(undefined, 'bold').text("Reporte de Rendiciones", pageWidth / 2, lastY, { align: 'center' });
    lastY += 8;
    doc.setFontSize(10).setFont(undefined, 'normal').text(`Fecha de Emisión: ${formatDate(new Date())}`, pageWidth / 2, lastY, { align: 'center' });
    lastY += 10;
    
    // Tabla de rendiciones
    const head = [['ID', 'Concepto', 'Responsable', 'Inicio', 'Estado', 'Aprobación', 'Saldo']];
    const body = rendiciones.map(v => [
      `#${v.codigo_rendicion || v.id}`,
      v.nombre_viaje,
      v.responsable_nombre || 'N/A',
      formatDate(v.fecha_inicio),
      v.cerrado_en ? 'Cerrado' : 'En Curso',
      (v.estado_aprobacion || 'N/A').replace('_', ' '),
      formatCurrency(v.saldo_adelanto_viaje || 0)
    ]);
    ...
    doc.autoTable({
      ...
      body: body,
      theme: 'striped',
      headStyles: { fillColor: [40, 56, 104], textColor: 255 },
      columnStyles: {
        0: { cellWidth: 15 },
        1: { cellWidth: 80 },
        2: { cellWidth: 50 },
      ...
      didDrawPage: function (data) {
        ...
        doc.setFontSize(8).setTextColor(150);
        doc.text(`Página ${doc.internal.pages.length - 1}`, data.settings.margin.left, doc.internal.pageSize.height - 5);
      }
    });

    doc.save(`Reporte_Rendiciones_${new Date().toISOString().split('T')[0]}.pdf`);
  };

  return {
    getReportData, // Exportamos la nueva función de datos
    generateCanvaStylePDF,
    generateAdminOperativoPDF,
    calculateAdminGastosPorTipo,
    calculateResumenGerencialPorTipo,
    prepararDetalleGastosParaLibroIVA,
    generateGastosPDF,
    generateRendicionesListPDF,
    generateCajaReportePDF,
    generateGastosExcel,
