...
async function fetchClientes() {
  ...
  try {
    ...
    errorMessage.value = `Error al cargar clientes: ${e.message}`;
  } finally {
    loading.value = false;
  }
}
...
function abrirFormularioParaCrear() {
  ...
  form.value = { id: null, nombre_cliente: '' };
  mostrarFormulario.value = true;
}

function abrirFormularioParaEditar(cliente) {
...
async function handleGuardado() {
  ...
  try {
    ...
    const { error } = esModoEdicion.value
      ? await supabase.from('clientes').update(payload).eq('id', form.value.id)
...
<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-districorr-primary">Gestionar Clientes</h1>
    ...
    <div v-if="mostrarFormulario" class="mb-8 p-4 bg-gray-50 rounded-lg shadow-inner border">
      <form @submit.prevent="handleGuardado" class="space-y-4">
        <h3 class="text-lg font-medium text-gray-800">{{ esModoEdicion ? 'Editar Cliente' : 'Nuevo Cliente' }}</h3>
        <div>
          <label for="nombre_cliente" class="form-label">Nombre del Cliente <span class="text-red-500">*</span></label>
          <input type="text" id="nombre_cliente" v-model="form.nombre_cliente" required class="form-input">
        </div>
    ...
    <!-- Tabla de Clientes -->
    <div v-if="loading" class="text-center py-10">Cargando clientes...</div>
      ...
      <table class="min-w-full divide-y divide-gray-200">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-if="clientes.length === 0">
            <td colspan="3" class="px-6 py-12 text-center text-gray-500">No hay clientes registrados.</td>
          </tr>
          <tr v-for="cliente in clientes" :key="cliente.id" class="hover:bg-gray-50">
            <td class="px-6 py-4 text-sm text-gray-500">{{ cliente.id }}</td>
            <td class="px-6 py-4 text-sm font-medium text-gray-900">{{ cliente.nombre_cliente }}</td>
            <td class="px-6 py-4 text-right text-sm font-medium space-x-2">
              <button @click="abrirFormularioParaEditar(cliente)" class="btn-admin-action-xs btn-yellow">Editar</button>
              <button @click="eliminarCliente(cliente.id)" class="btn-admin-action-xs btn-danger">Eliminar</button>
            </td>
          </tr>
        </tbody>
      </table>
