...
async function fetchTodosLosViajesConInfo() {
  ...
  try {
    ...
    if (filtroFechaDesdeViajes.value) query = query.gte('fecha_inicio', filtroFechaDesdeViajes.value);
    if (filtroFechaHastaViajes.value) {
      const fechaHastaObj = new Date(filtroFechaHastaViajes.value);
      fechaHastaObj.setUTCDate(fechaHastaObj.getUTCDate() + 1);
      query = query.lt('fecha_inicio', fechaHastaObj.toISOString().split('T')[0]);
    }
    ...
    query = query.order('cerrado_en', { ascending: true, nullsFirst: true })
                 .order('fecha_inicio', { ascending: false });

    const { data: viajesData, error: vistaError } = await query;

    if (vistaError) {
        console.error('AdminViajesListView: Error en la consulta a admin_viajes_con_responsable:', vistaError);
        throw vistaError;
    }
    
    console.log("AdminViajesListView: Datos de viajes CRUDOS obtenidos de la vista:", viajesData ? viajesData.length : 0, "viajes.");
    ...
    if (!viajesData || viajesData.length === 0) {
      ...
      console.log("AdminViajesListView: Datos de viajes PROCESADOS con totales:", todosLosViajes.value.length, "viajes.");
    }
