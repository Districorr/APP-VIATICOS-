...
// --- Lógica de Carga y Selección de Formato ---
onMounted(async () => {
...
function handleGastoGuardado(gastoGuardado) {
  alert('¡Gasto procesado con éxito!');
  if (gastoGuardado?.viaje_id) {
    router.push({ name: 'GastosListUser', query: { viajeId: gastoGuardado.viaje_id } });
  } else {
    router.push({ name: 'GastosListUser' });
  }
}
...
<template>
  <div class="min-h-screen bg-gray-100/50 py-6 sm:py-8 lg:py-12">
    <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
      ...
      <!-- 3. Pantalla de Selección de Formato (si hay más de uno) -->
      ...
      <!-- 4. Renderizado del Formulario (si se está editando o ya se seleccionó un formato) -->
      <div v-else>
        <GastoForm
